...
export async function POST(request: NextRequest) {
  try {
    const { codigo, nome, descricao, requer_numero_processo } = await request.json()
    ...
    const stmt = db.prepare(`
      INSERT INTO modalidades_compra (codigo, nome, descricao, ativo, requer_numero_processo)
      VALUES (?, ?, ?, 1, ?)
    `)
    
    const result = stmt.run(codigo.toUpperCase(), nome, descricao || null, requer_numero_processo ? 1 : 0)
