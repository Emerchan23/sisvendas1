...
export async function fecharAcerto(id: string): Promise<void> {
  try {
    // Get acerto details to access linhaIds
    const acertos = await getAcertos()
    const acerto = acertos.find(a => a.id === id)
    if (!acerto) {
      throw new Error('Acerto não encontrado')
    }
    
    // Update acerto status to fechado
    await api.acertos.update(id, { status: "fechado" })
    
    // Update settlement status of all sales in this acerto
    await setLinhasAcerto(acerto.linhaIds, id)
